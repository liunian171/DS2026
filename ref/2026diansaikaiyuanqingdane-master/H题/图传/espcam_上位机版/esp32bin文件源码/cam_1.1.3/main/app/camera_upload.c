...
#define CAMERA_FIRMWARE_VERSION "cam_1.1.3"
...
static SemaphoreHandle_t camera_stream_mutex;
 ...
 * @return 始终返回 ESP_OK；TCP_NODELAY 是可选优化，设置失败不会拒绝客户端连接。
 */
static esp_err_t configure_stream_socket(httpd_handle_t server, int socket_fd)
{
    ...
    if (setsockopt(socket_fd, IPPROTO_TCP, TCP_NODELAY, &no_delay, sizeof(no_delay)) != 0) {
        ESP_LOGW(TAG, "failed to enable TCP_NODELAY, continuing without it: errno=%d", errno);
    }
...
static esp_err_t camera_stream_handler(httpd_req_t *request)
{
    ...
    if (camera_stream_mutex == NULL) {
        return ESP_ERR_INVALID_STATE;
    }
    if (xSemaphoreTake(camera_stream_mutex, 0) != pdTRUE) {
        httpd_resp_set_status(request, "503 Service Unavailable");
        httpd_resp_set_type(request, "text/plain");
        return httpd_resp_send(
            request,
            "Video stream is already in use.",
    ...
    if (err != ESP_OK) {
        ...
        xSemaphoreGive(camera_stream_mutex);
        return err;
    }
...
{
    if (camera_stream_mutex == NULL) {
        camera_stream_mutex = xSemaphoreCreateMutex();
        if (camera_stream_mutex == NULL) {
            return ESP_ERR_NO_MEM;
        }
    }
