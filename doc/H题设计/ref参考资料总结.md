...
- 蓝牙: 4.2 BR/EDR + BLE
- 摄像头: 支持OV2640和OV7670(**部署实测确认OV2640**)
- 供电: 5V
- 功耗: 关闪光180mA, 深睡眠6mA
- IO口: 9个
- 串口速率: 默认115200bps
...
ESP32-CAM部署实测确认摄像头为**OV2640**（串口输出: `Detected OV2640 camera, using JPEG format`）。与规格书一致，与之前用户手误说的OV3660不符。
...
- 实际摄像头: **OV2640**（部署实测，串口确认）
- WiFi模式: **AP模式**（SSID:ESP32-CAM, 密码:12345678, IP:192.168.4.1）
- 串口: 115200bps调试输出
- 部署状态: ✅ 已编译烧录验证，MJPEG流正常
